function obj = trainer_backprop_quasi_newton_gaaNet( obj )   
    
    for i = 1:size(obj.B)
        obj.B{i} = zeros(size(obj.B{i}));
    end
    ...
    while batchSize*b < size(obj.X,1)-1
        ...
        for i = 1:numLayers
            ...
            input{i+1,1} = A{i} * obj.W{i}; %input to layer
        ...
        for i = 1:numLayers
            ...
            obj.W{i} = obj.W{i} - obj.options.learningRate * ...
                ...
                + ( A{i}' * errorOut{i} ) + ( A{i}' * my_hessian{i} ) ) ) ...
                / actBatchSize;
    ...
    if obj.options.log
        ...
        obj.logs.hessian{end+1,1} = my_hessian;
    end
